...
RUN zypper ref && \
    ...
    rm pubkey.gpg && \
    zypper --non-interactive ar -f https://dl.yarnpkg.com/rpm/ Yarn && \
    ...
    zypper --non-interactive in --no-recommends yarn && \
...
RUN yarn install --no-emoji --no-progress

RUN bundle exec rake --trace assets:precompile && \
